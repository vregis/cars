...
class OfferOptionsController extends Controller
{
	...
	public function actionAddMain($id,$mo=null)
	{
        ...
        if(isset($_POST['OfferOptions']))
        {
           ...
           if ($model->save()) {
                if($_POST['submit']=='addad')
                    $this->redirect(array('addaddition', 'id'=>$model->offer_id, 'mo'=>$_POST['OfferOptions']['main_option_id']));
                else
                   // $this->redirect(array('addmain', 'id' => $id, 'mo'=>$_POST['OfferOptions']['main_option_id']));
                    $this->redirect(array('index', 'id' => $id));
            }
        }
        ...
        if(empty($mo)){
            $model->attributes=array('offer_id'=>$id,'title'=>'Введите название',);
            ...
            $model=$this->loadModel($mo);
             $model->main_option_id=$mo;
        }

        $this->render('editmain',array(
            'model'=>$model
                    ));
	}
	...
	public function actionDelete($id)
	{
        ...
        if (!Yii::app()->user->isAdmin() && (Yii::app()->user->id != $model->offer->owner_id))
            return false;  
        if(empty($model->main_option))
            $model->delete();
        else{
            $model->deleteRelated($model->main_option_id);
            $model->delete();
        }
