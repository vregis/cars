...
class CAccessRule extends CComponent
{
	...
	{
		 // var_dump($this->isActionMatched($action),$this->isUserMatched($user),$this->isRoleMatched($user), $this->isIpMatched($ip),$this->isVerbMatched($verb),$this->isControllerMatched($controller),$this->isExpressionMatched($user),$this->allow,$action,$controller, $user,$ip,$verb,$user->getIsGuest(),$this->users);
		 //die('that\'s all');
		 //roman
