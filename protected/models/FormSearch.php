...
{
    public $location, $type, $keyword, $date_since, $date_for, $rating, $privates, $approved,$age_from,$age_to;
    public $lat, $lng;
    public $sort = 6;
    public $offset, $limit;
	...
	public function rules()
	{
		...
		return array(
			array('location, type, date_since, date_for, rating, keyword, age_from, age to', 'safe'),
		);
	}
	...
	public function findOffers()
	{
        ...
        if (!empty($this->keyword)) {
            $criteria->addSearchCondition('`t`.description','%'.$this->keyword.'%',false,'AND','LIKE');
        }
        //search by age
        //echo "Age";
        //var_dump($_POST);
        //var_dump($this->age_from);
        if (!empty($this->age_from) && !empty($this->age_to)) {
            $criteria->addCondition('    (`t`.age_from <= :age_from AND `t`.age_to >= :age_from)
                                      OR (`t`.age_from >= :age_from AND `t`.age_from <= :age_to )'
                                    );
            $criteria->params[':age_from'] = $this->age_from;
            $criteria->params[':age_to'] = $this->age_to;
        ...
        if (!empty($this->sort)) {            
            if ($this->sort & 2) $order['rating'] = '`t`.rating ASC';// '`t`.rating ASC'
            if ($this->sort & 8) $order['price'] = 'lp ASC'; //'`t`.price_hourly ASC';
        }
        $criteria->order = implode(', ', $order);
        //$criteria->order='lp DESC';
        $criteria->select ='`t`.*,min(offer_options.price_daily) as lp, max(offer_options.price_daily) as hp';
        $criteria->group='`t`.id';
        $criteria->join='LEFT JOIN offer_options ON `t`.id=offer_options.offer_id AND offer_options.main_option=1';
        //var_dump($criteria);
        //echo "\n\n sort\n\n";
        //var_dump($order);
